  ...
  <div class="page">
    ...
    <el-pagination class="pager" layout="total,prev,pager,next" :total="total" @current-change="p=>{q.pageNum=p;load()}"/>
      ...
      <el-tab-pane label="扫描结果" name="results">
        <div class="toolbar"><el-input v-model="resultQuery.keyword" placeholder="任务名称或编号" @keyup.enter.native="loadResults"/><el-button type="primary" @click="loadResults">查询</el-button></div>
...
export default {
  ...
  data: () => ({
    ...
    resultQuery: { keyword: '', pageNum: 1, pageSize: 20 }, resultRows: [], resultTotal: 0, resultLoading: false, resultVisible: false, selectedResult: null, timer: null
  }),
  computed: {
  ...
  watch: { activeTab (value) { if (value === 'results') this.loadResults() } },
  created () { this.activeTab = this.$route.query.tab === 'results' ? 'results' : 'tasks'; this.resultQuery.keyword = this.$route.query.taskNo || ''; this.load(); this.loadResults(); this.timer = setInterval(this.load, 4000) },
  ...
  methods: {
    ...
    repositoryLabel (row) { const app = row.application ? ' / ' + row.application : ''; const md = row.scanSourceType === 'MD' ? ` / ${{ OVERVIEW_DESIGN: '概要设计.md', DETAIL_DESIGN: '详细设计.md' }[row.mdDocumentType] || ''}` : ''; const zip = row.sourceType === 'UPLOAD' ? ` / 单项目ZIP：${row.originalFileName || '未上传'}` : ''; return `[${row.scanSourceType}] ${row.repositoryName}${md}${app}${zip}` },
