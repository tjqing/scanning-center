<template>
  <div class="page">
    ...
    <el-dialog :title="form.id?'编辑扫描任务（保存后生成新快照）':'新建扫描任务'" :visible.sync="visible" width="760px"><el-alert title="任务保存后将固化规则、扫描源参数和内置提示词，并生成不可变扫描清单" type="info" :closable="false" style="margin-bottom:16px"/><el-form label-width="120px"><el-form-item label="任务名称" required><el-input v-model="form.taskName"/></el-form-item><el-form-item label="扫描源" required><el-select v-model="form.repositoryId" filterable @change="sourceChanged"><el-option v-for="r in repos" :key="r.id" :value="r.id" :label="repositoryLabel(r)"/></el-select></el-form-item><el-form-item label="扫描规则" required><el-select v-model="form.ruleIds" multiple filterable><el-option v-for="r in availableRules" :key="r.id" :value="r.id" :label="r.ruleName+'（'+r.ruleType+'）'"/></el-select></el-form-item><el-form-item v-if="selectedScanSourceType==='MD'" label="应用版本" required><el-input v-model="form.versionNo" maxlength="6" placeholder="例如 202608"/><div class="hint">系统按扫描源所属应用和该版本从数据库查询文档。</div></el-form-item><el-form-item label="扫描路径"><el-input v-model="form.scanPathsInput" placeholder="/ 或 src,docs；使用英文逗号分隔"/></el-form-item><el-form-item label="文件类型"><el-input v-model="form.fileTypesInput" placeholder="java,js,vue,md,docx,pdf"/></el-form-item><el-form-item label="排除路径"><el-input v-model="form.excludePathsInput" placeholder=".git,node_modules,target"/></el-form-item><el-form-item label="任务说明"><el-input type="textarea" v-model="form.description"/></el-form-item><el-form-item v-if="!form.id" label="生成后启动"><el-switch v-model="form.executeImmediately"/></el-form-item></el-form><span slot="footer"><el-button @click="visible=false">取消</el-button><el-button type="primary" :loading="saving" @click="save">保存并生成清单</el-button></span></el-dialog>
...
const empty = () => ({ taskName: '', description: '', repositoryId: null, ruleIds: [], versionNo: '', scanPathsInput: '/', fileTypesInput: 'java,js,ts,vue,xml,yml,yaml,json,md,txt,sql,docx,pdf', excludePathsInput: '.git,node_modules,target', executeImmediately: false })
...
export default {
  ...
  methods: {
    ...
    sourceChanged () { this.form.ruleIds = []; if (this.selectedScanSourceType !== 'MD') this.form.versionNo = '' },
    ...
    async open (row) {
      ...
      if (row) {
        ...
        this.form = Object.assign(empty(), detail, { ruleIds: [], scanPathsInput: (scope.scanPaths || ['.']).map(item => item === '.' ? '/' : item).join(','), fileTypesInput: (scope.fileTypes || []).join(','), excludePathsInput: (scope.excludePaths || []).join(','), versionNo: scope.versionNo || '' })
        const snapshots = await taskApi.snapshots(row.id)
        if (snapshots.length) this.form.versionNo = snapshots[0].versionNo || this.form.versionNo
    ...
    async save () {
      ...
      if (this.selectedScanSourceType === 'MD' && !/^\d{6}$/.test(this.form.versionNo)) return this.$message.warning('MD扫描源任务版本必须为YYYYMM格式')
